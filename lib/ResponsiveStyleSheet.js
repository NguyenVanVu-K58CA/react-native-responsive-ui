    ...
    static create(styles: {}): StyleSheet.Styles {
        ...
        if(Device.isTablet) {
            ...
            for (const styleName in styles) {
                if (styles.hasOwnProperty(styleName)) {
                    const style = styles[styleName];
                    if (style.hasOwnProperty('landscape') && style.hasOwnProperty('portrait') && style.hasOwnProperty('phone')) {
                        const {landscape, portrait, phone,...rest} = style;
                        selectedStyles[styleName] = {...phone, ...rest};
                    } else if (style.hasOwnProperty('landscape') && style.hasOwnProperty('phone')) {
                        const {phone, landscape,...rest} = style;
                        selectedStyles[styleName] = {...phone, ...rest}
                    } else if (style.hasOwnProperty('portrait') && style.hasOwnProperty('phone')) {
                        const {phone, portrait, ...rest} = style;
                        selectedStyles[styleName] = {...phone, ...rest}
                    } else if (style.hasOwnProperty('phone')) {
                        const {phone, ...rest} = style;
                        selectedStyles[styleName] = {...phone, ...rest}
                    }
                      else 
                    {
